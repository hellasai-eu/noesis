-- Update user_has_course_tag_access to include enrollment-based access
...
AS $$
  SELECT 
    ...
    OR
    -- Students with TAG-based access (legacy)
    (
      EXISTS (
        SELECT 1 
        FROM courses c
    ...
    OR
    -- Students/Instructors with ENROLLMENT-based access (class → offering → course)
    EXISTS (
      SELECT 1
